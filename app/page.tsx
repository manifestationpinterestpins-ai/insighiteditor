...
const BookmarkIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" fill="currentColor" aria-label="Save" class="x1lliihq x1n2onr6 xyb1xck"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m20 21-8-7.56L4 21V3h16z"/></svg>
)
const InfoIcon = () => (
          ...
          <section className="flex flex-col items-center pt-4 pb-4 px-5">
            <div className="relative w-[130px] h-[230px] bg-zinc-900 rounded-xl overflow-hidden cursor-pointer group shadow-lg" onClick={() => { if (!locked) thumbnailInputRef.current?.click() }}>
              ...
              <input ref={thumbnailInputRef} type="file" accept="image/*" className="hidden" onChange={handleThumbnailUpload} />
                                    ...
                                    <div className="flex items-center justify-center gap-10 w-full mt-4 px-2">
              <div className="flex flex-col items-center gap-1 p-0.5"><HeartIcon /><span className="text-[10px] text-white mt-0.5">{insightsData.likes}</span></div>
              <div className="flex flex-col items-center gap-1 p-0.5"><CommentIcon /><span className="text-[10px] text-white mt-0.5">{insightsData.comments}</span></div>
              <div className="flex flex-col items-center gap-1 p-0.5"><RepostIcon /><span className="text-[10px] text-white mt-0.5">{insightsData.reposts}</span></div>
              <div className="flex flex-col items-center gap-1 p-0.5"><SendIcon /><span className="text-[10px] text-white mt-0.5">{insightsData.shares}</span></div>
              <div className="flex flex-col items-center gap-1 p-0.5"><BookmarkIcon /><span className="text-[10px] text-white mt-0.5">{insightsData.bookmarks}</span></div>
            </div>
          </section>
