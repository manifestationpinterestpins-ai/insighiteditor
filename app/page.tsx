...
const InfoIcon = () => (
  <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="text-gray-400">
    ...
    <circle cx="12" cy="7.7" r="1.25"/><path fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10.5 16.8h3M10.5 11H12v5.7"/>
  </svg>
)
...
const BottomSheet = ({ open, onClose }: { open: boolean; onClose: () => void }) => {
  ...
  useEffect(() => { if (!open) return; const h = (e: MouseEvent) => { if (sheetRef.current && !sheetRef.current.contains(e.target as Node)) onClose() }; const t = setTimeout(() => { document.addEventListener("mousedown", h); document.addEventListener("touchstart", h as any) }, 100); return () => { clearTimeout(t); document.removeEventListener("mousedown", h); document.removeEventListener("touchstart", h as any) } }, [open, onClose])
  return (
    ...
    <div ref={sheetRef} className="fixed left-0 right-0 bottom-0 z-[70] transition-transform duration-300 ease-out" style={{ transform: open ? "translateY(0)" : "translateY(100%)" }}>
      ...
      <div className="bg-[#1c1c1e] px-4 pb-8">
        <button className="w-full flex items-center justify-between py-3.5 active:opacity-60 transition-opacity" onClick={onClose}><div className="flex items-center gap-3.5"><div className="w-9 h-9 rounded-full bg-zinc-800 flex items-center justify-center"><BoostIcon /></div><span className="text-[14px] text-white">Boost this reel</span></div><ChevronRightIcon /></button>
        <div className="h-px bg-zinc-800" />
        <button className="w-full flex items-center justify-between py-3.5 active:opacity-60 transition-opacity" onClick={onClose}><div className="flex items-center gap-3.5"><div className="w-9 h-9 rounded-full bg-zinc-800 flex items-center justify-center"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg></div><span className="text-[14px] text-white">View on Edits</span></div><ChevronRightIcon /></button>
      </div><div className="bg-[#1c1c1e] pb-[env(safe-area-inset-bottom)]" />
    </div></>
  )
}

const LockMenu = ({ locked, onToggle, onOpenEditor, onLongPress }: { locked: boolean; onToggle: () => void; onOpenEditor: () => void; onLongPress: () => void }) => {
  const [open, setOpen] = useState(false); const menuRef = useRef<HTMLDivElement>(null); const longPressTimer = useRef<ReturnType<typeof setTimeout> | null>(null); const isLongPress = useRef(false)
    ...
    <div className="relative" ref={menuRef}>
      ...
      {open && (
        <div className="absolute right-0 top-10 w-[180px] bg-zinc-900 border border-zinc-700 rounded-2xl shadow-xl overflow-hidden z-50">
          <button className="w-full flex items-center gap-3 px-4 py-3 text-[12px] text-white hover:bg-zinc-800 transition-colors text-left" onClick={() => { setOpen(false); onOpenEditor() }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>Edit insights</button>
...
const GenderEditor = ({ menValue, onSave, locked }: { menValue: number; onSave: (n: number) => void; locked: boolean }) => {
  ...
  return <span className={`text-[12px] text-gray-300 ${locked ? "cursor-default" : "cursor-pointer hover:text-fuchsia-400"} transition-colors`} onClick={() => { if (!locked) { setValue(menValue.toFixed(1)); setEditing(true) } }}>{menValue.toFixed(1)}%</span>
}
...
const CountryNameEditor = ({ name, onSave, locked }: { name: string; onSave: (n: string) => void; locked: boolean }) => {
  ...
  return <span className={`text-[12px] text-gray-300 ${locked ? "cursor-default" : "cursor-pointer hover:text-fuchsia-400"} transition-colors`} onClick={() => { if (!locked) { setValue(name); setEditing(true) } }}>{name}</span>
}
...
const DraggableGraph = ({ data, onChange, locked }: { data: GraphPoint[]; onChange: (d: GraphPoint[]) => void; locked: boolean }) => {
  ...
  const padding = { top: 15, right: 10, bottom: 30, left: 35 }; const width = 380; const height = 170
  ...
  return (
    <div className="relative -mx-1">
      ...
      <svg ref={svgRef} viewBox={`0 0 ${width} ${height}`} className="w-full touch-none select-none" onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerLeave={handlePointerUp}>
        {yLabels.map((label, i) => <text key={`yt-${i}`} x={padding.left - 6} y={yPositions[i] + 4} textAnchor="end" fill={editingY === i ? "#d63bcd" : "#9ca3af"} fontSize="9" fontFamily="sans-serif" className={locked ? "cursor-default" : "cursor-pointer"} onClick={() => { if (locked) return; setEditingY(i); setEditingX(null); setEditValue(label) }}>{label}</text>)}
        {xLabels.map((label, i) => <text key={`xt-${i}`} x={xPositions[i]} y={height - 6} textAnchor="middle" fill={editingX === i ? "#d63bcd" : "#9ca3af"} fontSize="9" fontFamily="sans-serif" className={locked ? "cursor-default" : "cursor-pointer"} onClick={() => { if (locked) return; setEditingX(i); setEditingY(null); setEditValue(label) }}>{label}</text>)}
        <path d={buildPath(visibleThisReel)} fill="none" stroke="#d63bcd" strokeWidth={5} strokeLinecap="round" />
...
const DraggableRetentionGraph = ({ data, onChange, locked }: { data: RetentionPoint[]; onChange: (d: RetentionPoint[]) => void; locked: boolean }) => {
  ...
  const padding = { top: 15, right: 10, bottom: 30, left: 35 }; const width = 380; const height = 140
  const chartW = width - padding.left - padding.right; const chartH = height - padding.top - padding.bottom
  const getX = (i: number) => padding.left + (i / Math.max(data.length - 1, 1)) * chartW
  ...
  const handlePointerUp = () => setDragging(null); const lastIdx = data.length - 1
  ...
  return (
    <div className="relative -mx-1">
      ...
      <svg ref={svgRef} viewBox={`0 0 ${width} ${height}`} className="w-full touch-none select-none" onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerLeave={handlePointerUp}>
        {[0, 50, 100].map(t => <text key={t} x={padding.left - 5} y={getY(t) + 3} textAnchor="end" fill="#9ca3af" fontSize="9" fontFamily="sans-serif">{t === 0 ? "0" : `${t}%`}</text>)}
        {data[0] && <text x={getX(0)} y={height - 6} textAnchor="middle" fill="#9ca3af" fontSize="9" fontFamily="sans-serif">{data[0].time}</text>}
        {data[lastIdx] && <text x={getX(lastIdx)} y={height - 6} textAnchor="middle" fill={editingRightX ? "#d63bcd" : "#9ca3af"} fontSize="9" fontFamily="sans-serif" className={locked ? "cursor-default" : "cursor-pointer"} onClick={() => { if (locked) return; setRightXValue(data[lastIdx].time); setEditingRightX(true) }}>{data[lastIdx].time}</text>}
...
export default function ReelInsights() {
  ...
  useEffect(() => { try { const sl = localStorage.getItem("site-locked"); if (sl) setLocked(JSON.parse(sl)); const sp = localStorage.getItem("profile-activity"); if (sp) setProfileActivity(JSON.parse(sp)) } catch {} }, [])
  const toggleLock = () => { const n = !locked; setLocked(n); try { localStorage.setItem("site-locked", JSON.stringify(n)) } catch {} }
  useEffect(() => { const obs = new IntersectionObserver(entries => { if (entries[0].isIntersecting && !overviewTriggeredRef.current) { overviewTriggeredRef.current = true; setOverviewVisible(true) } }, { threshold: 0.25 }); if (overviewRef.current) obs.observe(overviewRef.current); return () => obs.disconnect() }, [])
  ...
  const ProgressBar = ({ percentage, color = "magenta", delay = 0 }: { percentage: number; color?: "magenta" | "violet" | "blue"; delay?: number }) => {
    ...
    return <div className="relative w-full h-[8px] bg-[#161a21] rounded-full overflow-hidden"><div className="absolute left-0 top-0 h-full rounded-full transition-all duration-700 ease-out" style={{ width: `${width}%`, backgroundColor: cs[color] }} /></div>
  }
  ...
  return (
    <div className="min-h-screen text-white font-sans antialiased overflow-x-hidden flex justify-center" style={{ backgroundColor: BG }}>
      ...
      <section className="flex flex-col items-center pt-4 pb-3 px-5">
        ...
        <p className="text-[11px] text-gray-400 mt-1">{insightsData.publishDate} · Duration {insightsData.videoDuration}</p>
      ...
      <div><div className="flex">
        {(["Overview", "Engagement", "Audience"] as const).map(tab => (
          <button key={tab} onClick={() => setMainTab(tab)} className={`flex-1 py-2.5 text-[13px] font-medium text-center relative transition-colors ${mainTab === tab ? "text-white" : "text-gray-500"}`}>
            {tab}{mainTab === tab && <div className="absolute bottom-0 left-1/4 right-1/4 h-[2px] bg-white rounded-full" />}
          </button>
        ))}
      </div></div>
        ...
        {mainTab === "Overview" && (
          <>
            <section ref={overviewRef} key={animationKey} className="px-4 pt-5 pb-4">
              <div className="flex items-center gap-2 mb-4"><h3 className="text-[15px] font-semibold">Summary</h3><button onClick={replayOverviewAnimation} className="focus:outline-none active:opacity-60 transition-opacity"><InfoIcon /></button></div>
              <div className="grid grid-cols-2 gap-2.5">
                <div className="bg-[#161a21] rounded-xl p-3.5"><span className="text-[11px] text-gray-400">Views</span><p className="text-[20px] font-bold text-white mt-0.5">{overviewVisible ? countedViews.toLocaleString("en-IN") : "0"}</p></div>
                <div className="bg-[#161a21] rounded-xl p-3.5"><span className="text-[11px] text-gray-400">Accounts reached</span><p className="text-[20px] font-bold text-white mt-0.5">{insightsData.accountsReached.toLocaleString("en-IN")}</p></div>
                <div className="bg-[#161a21] rounded-xl p-3.5"><span className="text-[11px] text-gray-400">Average watch time</span><p className="text-[20px] font-bold text-white mt-0.5">{insightsData.avgWatchTime}</p></div>
                <div className="bg-[#161a21] rounded-xl p-3.5"><span className="text-[11px] text-gray-400">Follows</span><InlineEditor value={profileActivity} isNumber locked={locked} className="text-[20px] font-bold text-white mt-0.5 block" onSave={val => { const n = Math.round(val); setProfileActivity(n); try { localStorage.setItem("profile-activity", JSON.stringify(n)) } catch {} }} /></div>
              </div>
            </section>
            ...
            <section className="px-4 py-5">
              <div className="flex items-center justify-between mb-4"><div className="flex items-center gap-2"><h3 className="text-[15px] font-semibold">Views</h3><InfoIcon /></div><span className="text-[15px] font-semibold">{insightsData.views.toLocaleString("en-IN")}</span></div>
              <div className="flex gap-2 mb-4">
                {(["All", "Followers", "Non-followers"] as const).map(filter => (<button key={filter} onClick={() => setViewsFilter(filter)} className={`px-3.5 py-[7px] rounded-full text-[11px] font-medium transition-all duration-200 ${viewsFilter === filter ? "bg-white text-black" : "bg-transparent text-gray-300 border border-zinc-700"}`}>{filter}</button>))}
              </div>
              ...
              <div className="space-y-3.5">
                {affectsData.map((item, i) => (
                  <div key={i} className="flex items-center justify-between">
                    <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-[#161a21] flex items-center justify-center">{item.icon}</div><span className="text-[13px] text-gray-300 font-medium">{item.label}</span></div>
            ...
            <section className="px-4 py-5">
              ...
              <div className="space-y-4">
                {insightsData.sourcesData.map((source, index) => (
                  <div key={source.name}><div className="flex justify-between mb-1.5"><span className="text-[12px] text-gray-400">{source.name}</span><span className="text-[12px] text-white font-medium">{source.percentage.toFixed(1)}%</span></div><ProgressBar percentage={source.percentage} delay={index * 100} /></div>
                ))}
              </div>
            </section>
            ...
            <section className="px-4 py-5">
              ...
              <button className="w-full flex items-center justify-between py-2 active:opacity-60 transition-opacity"><div className="flex items-center gap-3"><BoostIcon /><span className="text-[13px] text-white font-medium">Boost this Reel</span></div><ChevronRightIcon /></button>
            </section>
          </>
        )}
        ...
        {mainTab === "Engagement" && (
          <>
            <section className="px-4 py-5">
              ...
              {(() => {
                const svgW = 380; const svgH = 140; const pad = { top: 15, right: 10, bottom: 28, left: 35 }; const cW = svgW - pad.left - pad.right; const cH = svgH - pad.top - pad.bottom
                ...
                return (
                  <svg viewBox={`0 0 ${svgW} ${svgH}`} className="w-full">
                    {[0, 50, 100].map(t => <text key={t} x={pad.left - 5} y={getY(t) + 3} textAnchor="end" fill="#9ca3af" fontSize="9" fontFamily="sans-serif">{t === 0 ? "0" : `${t}%`}</text>)}
                    <text x={pad.left} y={svgH - 5} textAnchor="middle" fill="#9ca3af" fontSize="9" fontFamily="sans-serif">0:00</text>
                    <text x={svgW - pad.right} y={svgH - 5} textAnchor="middle" fill="#9ca3af" fontSize="9" fontFamily="sans-serif">{durStr}</text>
            ...
            <section className="px-4 py-5">
              ...
              <div className="flex items-center justify-between"><div className="flex items-center gap-2"><div className="w-[6px] h-[6px] rounded-full bg-red-500 shrink-0" /><span className="text-[12px] text-gray-400">Not monetising</span></div><button className="text-[12px] text-blue-500 font-medium active:opacity-60 transition-opacity">Add payment details</button></div>
            </section>
          </>
        )}
          ...
          <>
            ...
            <section className="px-4 py-5">
              ...
              <div className="flex gap-2 mb-5">
                {(["Age", "Country", "Gender"] as const).map(tab => (<button key={tab} onClick={() => setAudienceTab(tab === "Age" ? "Age" : tab === "Country" ? "Country" : "Gender")} className={`px-3.5 py-[7px] rounded-full text-[11px] font-medium transition-all duration-200 ${audienceTab === tab ? "bg-white text-black" : "bg-transparent text-gray-300 border border-zinc-700"}`}>{tab}</button>))}
              </div>

              {audienceTab === "Age" && (
                <div className="space-y-3">
                  {insightsData.ageData.map((age, index) => (
                    <div key={age.name} className="flex items-center gap-2.5"><span className="text-[11px] text-gray-400 w-[40px] shrink-0">{age.name}</span><div className="flex-1"><ProgressBar percentage={age.percentage} delay={index * 60} /></div><span className="text-[11px] text-white font-medium w-[40px] text-right">{age.percentage.toFixed(1)}%</span></div>
                  ))}
                </div>
              )}
              ...
              {audienceTab === "Country" && (
                <div className="space-y-3">
                  {insightsData.countryData.map((country, index) => (
                    <div key={index} className="flex items-center gap-2.5"><div className="w-[85px] shrink-0"><CountryNameEditor locked={locked} name={country.name} onSave={newName => { const uc = [...insightsData.countryData]; uc[index] = { ...uc[index], name: newName }; try { localStorage.setItem("country-names", JSON.stringify(uc.map(c => c.name))) } catch {}; saveData({ ...insightsData, countryData: uc }) }} /></div><div className="flex-1"><ProgressBar percentage={country.percentage} delay={index * 80} /></div><span className="text-[11px] text-white font-medium w-[40px] text-right">{country.percentage.toFixed(1)}%</span></div>
                  ))}
                </div>
              )}
              ...
              {audienceTab === "Gender" && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2.5"><span className="text-[11px] text-gray-400 w-[50px] shrink-0">Men</span><div className="flex-1"><ProgressBar percentage={insightsData.genderData.men} delay={0} /></div><div className="w-[50px] text-right"><GenderEditor locked={locked} menValue={insightsData.genderData.men} onSave={newMen => { const nw = parseFloat((100 - newMen).toFixed(1)); try { localStorage.setItem("gender-data", JSON.stringify({ men: newMen, women: nw })) } catch {}; saveData({ ...insightsData, genderData: { men: newMen, women: nw } }) }} /></div></div>
                  <div className="flex items-center gap-2.5"><span className="text-[11px] text-gray-400 w-[50px] shrink-0">Women</span><div className="flex-1"><ProgressBar percentage={insightsData.genderData.women} color="violet" delay={100} /></div><span className="text-[11px] text-white font-medium w-[50px] text-right">{insightsData.genderData.women.toFixed(1)}%</span></div>
                </div>
              )}
            </section>
          </>
